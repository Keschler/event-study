...
from src.data.market_data import compute_daily_returns, download_daily_prices
from src.reporting.report import summarize_car, write_report
from src.scraping.scrape_x import scrape_user_posts
from src.settings.config import load_config
from src.utils.dedupe import merge_dedupe
from src.utils.textproc import add_vader_sentiment, clean_text
...
def run_scrape(config: dict) -> Path:
    ...
    posts = scrape_user_posts(
        ...
        page_size=int(scrape_cfg.get("page_size", 70)),
        min_request_interval_s=float(
            scrape_cfg.get(
                "min_request_interval_s",
                max(1.0, float(scrape_cfg.get("sleep_s", 1.0))),
            )
        ),
        max_rate_limit_hits=int(scrape_cfg.get("max_rate_limit_hits", 5)),
        stop_on_rate_limit=bool(scrape_cfg.get("stop_on_rate_limit", False)),
    )
    print(posts)
